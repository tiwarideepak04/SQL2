...
join bank_account_transaction abc on 
...
select bat.account_number,bat.transaction_amount,bat.transcation_channel,bat.transaction_date
from bank_account_transaction bat
join bank_holidays bh
on (bat.transaction_date=bh.holiday) and (monthname(bat.transaction_date) like '%march%');
---------------------------------------------------
# Question 4:
# 4) Calculate the Bank accrued interest with respect to their RECURRING DEPOSITS  for any deposits older than 30 days . 


select bat.account_number,(bat.transaction_amount*0.07) 'bank accrued interest',
bat.transaction_date 
from bank_account_transaction bat
join bank_account_relationship_details bard
on (bat.account_number=bard.account_number) and 
(bard.account_type like '%deposits%')
where datediff(now(),bat.transaction_date)>30;
-------------------------------------------------------
...
join bank_account_transaction bat
on bad.Account_Number=bat.account_number
and bad.account_type like '%add-on%'
group by bad.account_type
having count(bat.Transaction_Date)=1;
...
natural left join bank_customer
