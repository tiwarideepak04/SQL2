...
select purchase_cost, sum(purchase_cost),
 estimate_sale_price, avg(estimate_sale_price), 
 (sum(purchase_cost) + avg(estimate_sale_price)) as net,
 month from bank_inventory_pricing where month = 2;
 
# Question 2:
# 2) Print average of estimated_sale_price with two decimals from bank_inventory_pricing table.

select format(avg(estimate_sale_price),2) from bank_inventory_pricing;
...
---------------------------------------------------------------------------------------------------------------------------------------
use hr;
select product, purchase_cost, estimate_sale_price from bank_inventory_pricing;
...
use hr;
alter table Bank_Inventory_pricing alter column purchase_cost set default 2000;
desc Bank_Inventory_pricing;

# Question 6:
# 6) Print unique records of bank_inventory_pricing without displaying the month.
-----------------------------------------------------------------------------------------------------------------------------------------
select product, quantity, price, purchase_cost, estimate_sale_price, month from Bank_Inventory_pricing;
...
update bank_inventory_pricing set purchase_cost = estimate_sale_price where purchase_cost is null and estimate_sale_price is not null;
select purchase_cost, avg(purchase_cost), estimate_sale_price from Bank_Inventory_pricing;

# Question 8:
# 8) Print the count of unique Products used in  Bank_inventory_pricing

select product, count(product), count(distinct(product)) from bank_inventory_pricing group by product;
...
select purchase_cost, sum(purchase_cost), month from Bank_Inventory_Pricing group by month;
...
select product, 
avg(estimate_sale_price) + (0.5*avg(estimate_sale_price))
from Bank_Inventory_pricing
group by product
...
select Product,
avg(ifnull(purchase_cost,greatest(Price,Estimate_sale_price))) 
from Bank_Inventory_pricing 
...
select product,
ifnull(avg(estimate_sale_price),purchase_cost)
from Bank_Inventory_pricing
group by product;
